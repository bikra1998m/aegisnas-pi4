...
The executable compatibility set includes Meraki AP tags, Palo Alto client OS, Airespace WLAN IDs, Arista profiling, and Meru AP IDs as inbound accounting context. Safe outbound additions include HP `Egress-VLANID`, Cambium `Cambium-Walled-Garden-State`, Colubris `Intercept` and `AVPair`, plus Pica8 and Nokia `AVPair` policy tags. These attributes remain opt-in through their vendor packs. Numeric role or privilege attributes and vendor-specific dynamic-ACL grammars remain planned until an operator template or certified encoding is configured.
