...
Run `make test-attribute-registry` and `make test-dictionary-release-profiles` before packaging or upgrading. Compare `/api/v1/system/dictionary-release-profiles` and `/api/v1/system/attribute-registry?limit=1` source hashes across HA nodes and reject mixed hashes or release profile IDs. Upstream dictionary changes require a reviewed generated diff; installed dictionary paths may be scanned for deployment coverage but cannot mutate the embedded packet registry. Full procedure: `attribute-registry.md` and `dictionary-release-profiles.md`.
