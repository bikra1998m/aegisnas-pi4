...
The response lists the supported controller adapters, whether each adapter is native or contract-based, the selected platform's capabilities, required site or network identifier, credential environment readiness, runtime sync status, and setup warnings. Cisco, Ruckus, and MikroTik readiness check `api_username_env` and `api_password_env`; token-based adapters check `api_token_env`. Aruba and FortiGate use `radius_profile` as an existing controller RADIUS profile; Ruckus uses it as the existing SmartZone authentication service name. Mist and MikroTik enterprise WLAN readiness requires `radius_server`, `radius_secret_env`, and the named secret in the process environment.
