...
- controller sync payloads include adapter capabilities and a desired-state hash; Cisco ISE ERS, Aruba Central Classic, and Juniper Mist site WLANs have native read/reconcile clients, while Ruckus, Fortinet, MikroTik, UniFi, and generic REST use declared sync contracts
...
This phase now delivers real runtime integrations for the declared production paths while still being honest about product boundaries. Cisco ISE, Aruba Central, and Juniper Mist execute provider-native resource operations; the remaining controller adapters are vendor-neutral sync contracts rather than promises of one-to-one feature parity with every vendor controller API.
