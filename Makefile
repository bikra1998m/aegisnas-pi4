...
test-radius-sql-accounting:
	...
	go test -p=1 -timeout=600s ./internal/adminapi -run 'HandleGetSQLAccounting|HandleReconcileSQLAccounting|OpenAPIAndSupportBundleIncludeSQLAccountingAndOrdering|AuthorizeSQLAccounting|ProductionReadinessIncludesSQLAccounting' -count=1
...
test-radius-accounting-ordering:
	...
	go test -p=1 -timeout=600s ./internal/adminapi -run 'AccountingOrdering|OpenAPIAndSupportBundleIncludeSQLAccountingAndOrdering|AuthorizeSQLAccounting|ProductionReadinessIncludesSQLAccounting' -count=1
