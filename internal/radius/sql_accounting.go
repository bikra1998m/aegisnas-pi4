	...
	report := SQLAccountingReport{
		...
		FreeRADIUS: SQLAccountingFreeRADIUSSurface{
			...
			Attributes: []string{
				...
				"Framed-IPv6-Address", "Framed-IPv6-Prefix", "Delegated-IPv6-Prefix", "Framed-Protocol",
				"Acct-Multi-Session-Id", "Acct-Link-Count", "Service-Type", "Class",
			},
...
func ReconcileSQLAccounting(ctx context.Context, cfg *config.Config, batchSize int) (SQLAccountingReconcileReport, error) {
	...
	if err := PruneAccountingIPAssignmentEvidence(cfg, time.Now().UTC()); err != nil {
		report.Status = "degraded"
		report.Message = err.Error()
		return report, err
	}
