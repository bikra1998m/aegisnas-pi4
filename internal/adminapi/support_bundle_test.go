...
func TestHandleDownloadSupportBundle(t *testing.T) {
	...
	require.Contains(t, entries, "api/accounting-ingest-spool.json")
	require.Contains(t, entries, "api/sql-accounting.json")
	require.Contains(t, entries, "api/accounting-ordering.json")
	require.Contains(t, entries, "api/accounting-counters.json")
	...
	require.Contains(t, entries, "api/policy-simulation-analyses.json")
	...
	assert.Contains(t, string(entries["api/accounting-ingest-spool.json"]), "\"report\"")
	assert.Contains(t, string(entries["api/sql-accounting.json"]), "\"report\"")
	assert.Contains(t, string(entries["api/accounting-ordering.json"]), "\"report\"")
	assert.Contains(t, string(entries["api/accounting-counters.json"]), "\"report\"")
	assert.Contains(t, string(entries["api/accounting-ip.json"]), "\"report\"")
	assert.Contains(t, string(entries["api/accounting-services.json"]), "\"report\"")
	assert.Contains(t, string(entries["api/dac-client.json"]), "\"report\"")
	assert.Contains(t, string(entries["api/dac-client-history.json"]), "\"summary\"")
	...
	assert.Contains(t, string(entries["api/policy-engine.json"]), "\"schema_version\"")
	assert.Contains(t, string(entries["api/policy-sets.json"]), "\"schema_version\"")
	assert.Contains(t, string(entries["api/policy-simulation-analyses.json"]), "[")
...
func TestHandleGetSupportBundleSummary(t *testing.T) {
	...
	assert.Contains(t, payload.ArchiveEntries, "api/accounting-ingest-spool.json")
	assert.Contains(t, payload.ArchiveEntries, "api/accounting-ordering.json")
	assert.Contains(t, payload.ArchiveEntries, "api/accounting-ip.json")
	assert.Contains(t, payload.ArchiveEntries, "api/dac-client.json")
	assert.Contains(t, payload.ArchiveEntries, "api/dac-client-history.json")
	assert.Contains(t, payload.ArchiveEntries, "api/policy-engine.json")
	assert.Contains(t, payload.ArchiveEntries, "api/policy-sets.json")
	assert.Contains(t, payload.ArchiveEntries, "api/policy-simulation-analyses.json")
	...
	assert.Contains(t, payload.APICaptures, "Local accounting ingest spool")
	assert.Contains(t, payload.APICaptures, "Accounting idempotency and ordering")
	assert.Contains(t, payload.APICaptures, "IPv6 prefix and route accounting")
	assert.Contains(t, payload.APICaptures, "Outbound dynamic authorization client")
	assert.Contains(t, payload.APICaptures, "Outbound dynamic authorization history")
	assert.Contains(t, payload.APICaptures, "Versioned policy set governance")
	assert.Contains(t, payload.APICaptures, "Policy simulation analyses")
