...
func RecordUpstreamAAAHistory(serverName, address string, authPort, acctPort int, status, message, responseCode string, latencyMs int64, supportsStatusServer bool, checkedAt string) error {
	...
	if _, err := DB.Exec(`INSERT INTO upstream_aaa_history (server_name, address, auth_port, acct_port, status, message, response_code, latency_ms, supports_status_server, checked_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		strings.TrimSpace(responseCode),
		latencyMs,
		boolToSQLite(supportsStatusServer),
		strings.TrimSpace(checkedAt),
	); err != nil {
...
func ListUpstreamAAAHistory(serverName, status string, limit int) ([]UpstreamAAAHistoryRecord, error) {
	...
	query := `SELECT id, COALESCE(server_name, ''), COALESCE(address, ''), COALESCE(auth_port, 0), COALESCE(acct_port, 0),
		...
		COALESCE(supports_status_server, 0), COALESCE(checked_at, ''), COALESCE(created_at, '')
	...
	for rows.Next() {
		...
		var supports int
		if err := rows.Scan(&item.ID, &item.ServerName, &item.Address, &item.AuthPort, &item.AcctPort, &item.Status, &item.Message, &item.ResponseCode, &item.LatencyMs, &supports, &item.CheckedAt, &item.CreatedAt); err != nil {
