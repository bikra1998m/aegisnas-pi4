...
func GetHAHistoryStats() (HAHistoryStats, error) {
	...
	err := DB.QueryRow(`SELECT
		COUNT(*),
		COALESCE(SUM(CASE WHEN event_type = 'failover' AND status = 'promoted' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'failover' AND status = 'returned' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'peer_health' AND status = 'failed' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'peer_health' AND status = 'recovered' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'vip_lease' AND status = 'acquired' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'vip_lease' AND status = 'preempted' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'vip_lease' AND status = 'released' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'vip_announcement' AND status = 'sent' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'vip_announcement' AND status = 'failed' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'replication_publish' AND status = 'success' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'replication_publish' AND status = 'failed' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'replication_freshness' AND status = 'stale' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'replication_stage' AND status = 'staged' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN event_type = 'replication_activate' AND status = 'activated' THEN 1 ELSE 0 END), 0),
