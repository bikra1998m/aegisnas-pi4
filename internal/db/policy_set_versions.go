...
func ListPolicySetVersions(limit int) ([]PolicySetVersion, error) {
	...
	rows, err := DB.Query(`SELECT v.id, v.set_key, v.version, v.status, COALESCE(v.description, ''),
		...
		FROM policy_set_versions v ORDER BY v.set_key, v.version DESC LIMIT ?`, limit)
	if err != nil {
		if tableMissing(err) {
			return nil, nil
		}
...
func GetActivePolicySetVersion(setKey string) (*PolicySetVersion, error) {
	...
	return getPolicySetVersionByStatusQuery(DB, setKey, PolicySetStatusActive)
}
...
func ListPolicySetActivationEvents(limit int) ([]PolicySetActivationEvent, error) {
	...
	rows, err := DB.Query(`SELECT id, version_id, COALESCE(previous_version_id, 0), event_type, status,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.VersionID, &item.PreviousVersionID, &item.EventType, &item.Status, &item.Actor, &item.Summary, &item.DetailsJSON, &item.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, item)
	}
...
func RecordPolicySetSimulation(record PolicySetSimulation) error {
	...
	_, err := DB.Exec(`INSERT INTO policy_set_simulations (
		version_id, evaluation_id, policy_sha256, request_hash, decision, allowed, quarantine,
		conflict_count, matched_rule_count, trace_node_count, actor, result_json
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		record.VersionID, record.EvaluationID, record.PolicySHA256, record.RequestHash, record.Decision, record.Allowed, record.Quarantine,
...
func createPolicySetVersionTx(tx *sql.Tx, req CreatePolicySetVersionRequest) (PolicySetVersion, error) {
	...
	var nextVersion int
	if err := tx.QueryRow(`SELECT COALESCE(MAX(version), 0) + 1 FROM policy_set_versions WHERE set_key = ?`, req.SetKey).Scan(&nextVersion); err != nil {
		return PolicySetVersion{}, err
	}
	createdAt := time.Now().UTC().Format(time.RFC3339)
	args := []any{
		req.SetKey, nextVersion, req.Status, nullIfEmpty(req.Description), intOrNull(req.ParentVersionID), intOrNull(req.RollbackOfVersionID),
	...
	insertSQL := `INSERT INTO policy_set_versions (
		set_key, version, status, description, parent_version_id, rollback_of_version_id,
...
func MarkPolicySetVersionActiveTx(tx *sql.Tx, versionID int, actor, note string) (previousID int, err error) {
	...
	active, err := getPolicySetVersionByStatusTx(tx, version.SetKey, PolicySetStatusActive)
	if err != nil {
		return 0, err
	}
	now := time.Now().UTC().Format(time.RFC3339)
	...
	if err := insertPolicySetActivationEventTx(tx, versionID, previousID, "activated", "active", actor, defaultPolicySetSummary(note, "Policy set version activated."), map[string]any{"previous_version_id": previousID}); err != nil {
		return 0, err
	}
...
func getPolicySetVersionQuery(q interface {
	...
	row := q.QueryRow(`SELECT v.id, v.set_key, v.version, v.status, COALESCE(v.description, ''),
		COALESCE(v.parent_version_id, 0), COALESCE(v.rollback_of_version_id, 0), v.content_json,
		v.content_sha256, v.policy_sha256, v.rule_count, v.child_set_count, v.max_depth,
		v.approval_required, v.min_approvals, COALESCE(v.created_by, ''), v.created_at,
		COALESCE(v.submitted_by, ''), COALESCE(v.submitted_at, ''), COALESCE(v.activated_by, ''),
...
func getPolicySetVersionByStatusQuery(q interface {
	...
	row := q.QueryRow(`SELECT v.id, v.set_key, v.version, v.status, COALESCE(v.description, ''),
		...
		COALESCE(v.rejected_at, ''), COALESCE(v.rejection_reason, ''), COALESCE(v.activation_note, ''),
		(SELECT COUNT(*) FROM policy_set_approvals a WHERE a.version_id = v.id)
		FROM policy_set_versions v WHERE v.set_key = ? AND v.status = ? ORDER BY v.version DESC LIMIT 1`, normalizePolicySetKey(setKey), status)
	item, err := scanPolicySetVersion(row)
	if err != nil {
		if tableMissing(err) || err == sql.ErrNoRows {
			return nil, nil
...
func scanPolicySetVersion(row interface {
	...
	err := row.Scan(&item.ID, &item.SetKey, &item.Version, &item.Status, &item.Description,
...
func insertPolicySetActivationEventTx(tx *sql.Tx, versionID, previousVersionID int, eventType, status, actor, summary string, details any) error {
	...
	_, err = tx.Exec(`INSERT INTO policy_set_activation_events (
		version_id, previous_version_id, event_type, status, actor, summary, details_json
	) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		versionID, intOrNull(previousVersionID), strings.TrimSpace(eventType), strings.TrimSpace(status),
