...
export default function CrudPage({
  ...
  const initialForm = useMemo(
    () =>
      Object.fromEntries(
        fields.map((field) => [field.name, emptyValue(field)]),
      ),
    [fields],
  );
  ...
  const openEdit = (item: any) => {
    ...
    setForm(
      Object.fromEntries(
        fields.map((field) => [field.name, valueForEdit(item, field)]),
      ),
    );
    setShowModal(true);
  };
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(
        "Change staged. Review and apply it from the pending changes bar.",
      );
      setShowModal(false);
  ...
  const remove = async (item: any) => {
    if (
      !confirm(
        `Delete ${itemName} "${item.name || item.username || item.code || item.id}"?`,
      )
    )
      return;
    setError("");
    try {
      ...
      setMessage(
        "Delete staged. Review and apply it from the pending changes bar.",
      );
      window.dispatchEvent(new Event("staged-changes-updated"));
    } catch (err: any) {
              ...
              items.map((item) => (
                <tr key={item.id}>
                  {columns.map((column) => (
                    <td
                      key={column.key}
                      className="max-w-xs px-5 py-4 text-sm text-gray-800"
                    >
                      {column.render
                        ? column.render(item)
                  ...
                  <td className="px-5 py-4 text-right text-sm">
                    <button
                      ...
                      className="mr-3 text-sky-700 hover:text-sky-900"
                    >
                      Edit
                    </button>
                    <button
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl">
            <h3 className="mb-4 text-lg font-bold">
              {editing ? `Edit ${itemName}` : `Add ${itemName}`}
            </h3>
