...
type VoucherAnalyticsExportRuntime = {
  component: string;
  status: string;
  message: string;
  updated_at: string;
...
type VoucherAnalyticsExportArtifact = {
  name: string;
  path: string;
  format: string;
  size_bytes: number;
...
type VoucherRedemptionAnalyticsExportArtifact = {
  name: string;
  path: string;
  format: string;
  size_bytes: number;
...
type VoucherExpiryAnalyticsExportRuntime = {
  component: string;
  status: string;
  message: string;
  updated_at: string;
...
type VoucherExpiryAnalyticsExportArtifact = {
  name: string;
  path: string;
  format: string;
  size_bytes: number;
...
type GuestSponsorAnalyticsExportArtifact = {
  name: string;
  path: string;
  format: string;
  size_bytes: number;
...
export default function Backups() {
  ...
  const [voucherAnalyticsExportRuntime, setVoucherAnalyticsExportRuntime] =
    useState<VoucherAnalyticsExportRuntime | null>(null);
  ...
  const [
    voucherRedemptionAnalyticsExportArtifacts,
    setVoucherRedemptionAnalyticsExportArtifacts,
  ] = useState<VoucherRedemptionAnalyticsExportArtifact[]>([]);
  const [
    voucherExpiryAnalyticsExportRuntime,
    setVoucherExpiryAnalyticsExportRuntime,
  ] = useState<VoucherExpiryAnalyticsExportRuntime | null>(null);
  const [
    voucherExpiryAnalyticsExportArtifacts,
    setVoucherExpiryAnalyticsExportArtifacts,
  ] = useState<VoucherExpiryAnalyticsExportArtifact[]>([]);
  ...
  const [
    guestSponsorAnalyticsExportArtifacts,
    setGuestSponsorAnalyticsExportArtifacts,
  ] = useState<GuestSponsorAnalyticsExportArtifact[]>([]);
  ...
  const [
    loadingVoucherExpiryAnalyticsExports,
    setLoadingVoucherExpiryAnalyticsExports,
  ] = useState(false);
  ...
  const [
    ...
    setLoadingGuestSponsorAnalyticsExports,
  ] = useState(false);
  ...
  const loadVoucherAnalyticsExports = async (announce = false) => {
    if (announce) {
      setError("");
      setMessage("");
    }
    setLoadingVoucherAnalyticsExports(true);
    try {
      const { data } = await api.get("/system/voucher-analytics-exports");
      setVoucherAnalyticsExportRuntime(data.runtime || null);
      setVoucherAnalyticsExportArtifacts(data.exports || []);
      if (announce) {
        setMessage("Scheduled voucher analytics exports refreshed.");
      }
    } catch (err: any) {
      setError(
        err.response?.data ||
          err.message ||
          "Could not load scheduled voucher analytics exports.",
      );
    } finally {
  ...
  const loadVoucherRedemptionAnalyticsExports = async (announce = false) => {
    if (announce) {
      setError("");
      setMessage("");
    }
    setLoadingVoucherRedemptionAnalyticsExports(true);
    try {
      const { data } = await api.get(
        "/system/voucher-redemption-analytics-exports",
      );
      setVoucherRedemptionAnalyticsExportRuntime(data.runtime || null);
      setVoucherRedemptionAnalyticsExportArtifacts(data.exports || []);
      if (announce) {
        setMessage(
          "Scheduled voucher redemption analytics exports refreshed.",
        );
      }
    } catch (err: any) {
      setError(
        err.response?.data ||
          err.message ||
          "Could not load scheduled voucher redemption analytics exports.",
      );
    } finally {
      setLoadingVoucherRedemptionAnalyticsExports(false);
    }
  };

  const loadVoucherExpiryAnalyticsExports = async (announce = false) => {
    if (announce) {
      setError("");
      setMessage("");
    }
    setLoadingVoucherExpiryAnalyticsExports(true);
    try {
      const { data } = await api.get("/system/voucher-expiry-analytics-exports");
      setVoucherExpiryAnalyticsExportRuntime(data.runtime || null);
      setVoucherExpiryAnalyticsExportArtifacts(data.exports || []);
      if (announce) {
        setMessage("Scheduled voucher expiry analytics exports refreshed.");
      }
    } catch (err: any) {
      setError(
        err.response?.data ||
          err.message ||
          "Could not load scheduled voucher expiry analytics exports.",
      );
    } finally {
      setLoadingVoucherExpiryAnalyticsExports(false);
    }
  };
  ...
  const loadGuestSponsorAnalyticsExports = async (announce = false) => {
    ...
    setLoadingGuestSponsorAnalyticsExports(true);
    try {
      const { data } = await api.get("/system/guest-sponsor-analytics-exports");
      setGuestSponsorAnalyticsExportRuntime(data.runtime || null);
      setGuestSponsorAnalyticsExportArtifacts(data.exports || []);
      if (announce) {
        setMessage("Scheduled guest sponsor analytics exports refreshed.");
      }
    } catch (err: any) {
      setError(
        err.response?.data ||
          err.message ||
          "Could not load scheduled guest sponsor analytics exports.",
      );
    } finally {
      setLoadingGuestSponsorAnalyticsExports(false);
    }
  };
  ...
  const downloadScheduledVoucherAnalyticsExport = async (
    artifact: VoucherAnalyticsExportArtifact,
  ) => {
    setError("");
    setMessage("");
    setBusyAction(`scheduled-voucher-analytics-${artifact.name}`);
    try {
      const response = await api.get(
        `/system/voucher-analytics-exports/download?name=${encodeURIComponent(artifact.name)}`,
        { responseType: "blob" },
      );
      const { data, headers } = response;
      const url = URL.createObjectURL(data);
      ...
      URL.revokeObjectURL(url);
      setMessage(
        `Scheduled voucher analytics export ${artifact.name} downloaded.`,
      );
    } catch (err: any) {
      setError(
        err.response?.data ||
  ...
  const downloadScheduledVoucherRedemptionAnalyticsExport = async (
    artifact: VoucherRedemptionAnalyticsExportArtifact,
  ) => {
    setError("");
    setMessage("");
    setBusyAction(`scheduled-voucher-redemption-analytics-${artifact.name}`);
    try {
      const response = await api.get(
        `/system/voucher-redemption-analytics-exports/download?name=${encodeURIComponent(artifact.name)}`,
        { responseType: "blob" },
      );
      const { data, headers } = response;
      const url = URL.createObjectURL(data);
      ...
      URL.revokeObjectURL(url);
      setMessage(
        `Scheduled voucher redemption analytics export ${artifact.name} downloaded.`,
      );
    } catch (err: any) {
      setError(
        err.response?.data ||
          err.message ||
          "Could not download scheduled voucher redemption analytics export.",
      );
    } finally {
      setBusyAction("");
    }
  ...
  const downloadScheduledVoucherExpiryAnalyticsExport = async (
    artifact: VoucherExpiryAnalyticsExportArtifact,
  ) => {
    setError("");
    setMessage("");
    setBusyAction(`scheduled-voucher-expiry-analytics-${artifact.name}`);
    try {
      const response = await api.get(
        `/system/voucher-expiry-analytics-exports/download?name=${encodeURIComponent(artifact.name)}`,
        { responseType: "blob" },
      );
      const { data, headers } = response;
      const url = URL.createObjectURL(data);
      ...
      URL.revokeObjectURL(url);
      setMessage(
        `Scheduled voucher expiry analytics export ${artifact.name} downloaded.`,
      );
    } catch (err: any) {
      setError(
        err.response?.data ||
          err.message ||
          "Could not download scheduled voucher expiry analytics export.",
      );
    } finally {
      setBusyAction("");
    }
  ...
  const downloadScheduledGuestSponsorAnalyticsExport = async (
    artifact: GuestSponsorAnalyticsExportArtifact,
  ) => {
    setError("");
    setMessage("");
    setBusyAction(`scheduled-guest-sponsor-analytics-${artifact.name}`);
    try {
      const response = await api.get(
        `/system/guest-sponsor-analytics-exports/download?name=${encodeURIComponent(artifact.name)}`,
        { responseType: "blob" },
      );
      const { data, headers } = response;
      const url = URL.createObjectURL(data);
      ...
      URL.revokeObjectURL(url);
      setMessage(
        `Scheduled guest sponsor analytics export ${artifact.name} downloaded.`,
      );
    } catch (err: any) {
      setError(
        err.response?.data ||
          err.message ||
          "Could not download scheduled guest sponsor analytics export.",
      );
    } finally {
      setBusyAction("");
    }
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-md border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1">
                    Keep recurring voucher inventory and expiry snapshots on
                    disk so stock, utilization, and expiration reviews do not
                    depend on a one-off download.
                  </div>
                </div>
                <button
                  onClick={() => void loadVoucherAnalyticsExports(true)}
                  disabled={loadingVoucherAnalyticsExports || busyAction !== ""}
                  className="rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50"
                >
                  {loadingVoucherAnalyticsExports
                    ? "Refreshing..."
                    : "Refresh Scheduled Voucher Analytics Exports"}
                </button>
              </div>
              {voucherAnalyticsExportRuntime ? (
                <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                  <div>
                    <span className="font-medium text-slate-900">Runtime:</span>{" "}
                    {voucherAnalyticsExportRuntime.status} /{" "}
                    {voucherAnalyticsExportRuntime.message}
                  </div>
                  <div className="mt-1">
                    Format{" "}
                    {String(
                      voucherAnalyticsExportRuntime.details?.format || "json",
                    )}
                    , every{" "}
                    {String(
                      voucherAnalyticsExportRuntime.details?.interval_minutes ||
                        0,
                    )}{" "}
                    minutes, retain{" "}
                    {String(
                      voucherAnalyticsExportRuntime.details?.retention_count ||
                        0,
                    )}
                    , directory{" "}
                    {String(
                      voucherAnalyticsExportRuntime.details?.directory ||
                        "unset",
                    )}
                    .
                  </div>
                  <div className="mt-1">
                    Window{" "}
                    {String(
                      voucherAnalyticsExportRuntime.details?.window_hours || 720,
                    )}{" "}
                    hours with{" "}
                    {String(
                      voucherAnalyticsExportRuntime.details?.bucket_count || 30,
                    )}{" "}
                    buckets.
                  </div>
                  {voucherAnalyticsExportRuntime.details?.last_export_at ? (
                    <div className="mt-1">
                      Last export{" "}
                      {String(
                        voucherAnalyticsExportRuntime.details.last_export_at,
                      )}
                      {voucherAnalyticsExportRuntime.details?.next_due_at
                        ? `, next due ${String(voucherAnalyticsExportRuntime.details.next_due_at)}`
                        : ""}
                      .
                    </div>
                  ) : null}
                ...
                <div className="mt-3 rounded-md border border-dashed border-gray-300 px-3 py-4 text-xs text-gray-500">
                  No scheduled voucher analytics export runtime has been
                  recorded yet.
                </div>
              )}
              {voucherAnalyticsExportArtifacts.length === 0 ? (
                <div className="mt-3 text-xs text-gray-500">
                  No scheduled voucher analytics export artifacts are present
                  yet.
                </div>
              ) : (
                <div className="mt-3 overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 text-xs">
                    ...
                    <tbody className="divide-y divide-gray-100 bg-white">
                      {voucherAnalyticsExportArtifacts.map((artifact) => (
                        <tr key={artifact.name}>
                          <td className="px-3 py-2 text-gray-600">
                            {artifact.created_at}
                          </td>
                          ...
                          <td className="px-3 py-2">
                            <button
                              onClick={() =>
                                void downloadScheduledVoucherAnalyticsExport(
                                  artifact,
                                )
                              }
                              disabled={busyAction !== ""}
            ...
            <div className="rounded-md border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1">
                    Keep recurring voucher redemption behavior snapshots on
                    disk so first-use delay, reuse, and voucher-session traffic
                    reviews do not depend on a one-off export.
                  </div>
                </div>
                <button
                  onClick={() =>
                    void loadVoucherRedemptionAnalyticsExports(true)
                  }
                  disabled={
                    loadingVoucherRedemptionAnalyticsExports ||
                    busyAction !== ""
                ...
                >
                  {loadingVoucherRedemptionAnalyticsExports
                    ? "Refreshing..."
                    : "Refresh Scheduled Voucher Redemption Analytics Exports"}
                </button>
              </div>
              {voucherRedemptionAnalyticsExportRuntime ? (
                <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                  <div>
                    <span className="font-medium text-slate-900">Runtime:</span>{" "}
                    {voucherRedemptionAnalyticsExportRuntime.status} /{" "}
                    {voucherRedemptionAnalyticsExportRuntime.message}
                  </div>
                  <div className="mt-1">
                    Format{" "}
                    {String(
                      voucherRedemptionAnalyticsExportRuntime.details?.format ||
                        "json",
                    )}
                    , every{" "}
                    {String(
                      voucherRedemptionAnalyticsExportRuntime.details
                        ?.interval_minutes || 0,
                    )}{" "}
                    minutes, retain{" "}
                    {String(
                  ...
                  {voucherRedemptionAnalyticsExportRuntime.details
                    ...
                    <div className="mt-1">
                      ...
                      {voucherRedemptionAnalyticsExportRuntime.details
                        ?.next_due_at
                        ? `, next due ${String(voucherRedemptionAnalyticsExportRuntime.details.next_due_at)}`
                        : ""}
                      .
                    </div>
                  ) : null}
                ...
                <div className="mt-3 rounded-md border border-dashed border-gray-300 px-3 py-4 text-xs text-gray-500">
                  No scheduled voucher redemption analytics export runtime has
                  been recorded yet.
                </div>
              )}
              {voucherRedemptionAnalyticsExportArtifacts.length === 0 ? (
                <div className="mt-3 text-xs text-gray-500">
                  No scheduled voucher redemption analytics export artifacts are
                  present yet.
                </div>
              ) : (
                <div className="mt-3 overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 text-xs">
                    ...
                    <tbody className="divide-y divide-gray-100 bg-white">
                      {voucherRedemptionAnalyticsExportArtifacts.map(
                        (artifact) => (
                          <tr key={artifact.name}>
                            <td className="px-3 py-2 text-gray-600">
                              {artifact.created_at}
                            ...
                            <td className="px-3 py-2">
                              <button
                                onClick={() =>
                                  void downloadScheduledVoucherRedemptionAnalyticsExport(
                                    artifact,
                                  )
                                }
                                disabled={busyAction !== ""}
            ...
            <div className="rounded-md border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1">
                    Keep recurring voucher expiry horizon snapshots on disk so
                    at-risk unused inventory and remaining use capacity reviews
                    do not depend on a one-off export.
                  </div>
                </div>
                <button
                  onClick={() => void loadVoucherExpiryAnalyticsExports(true)}
                  disabled={
                    loadingVoucherExpiryAnalyticsExports || busyAction !== ""
                  }
                  className="rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50"
                >
                  {loadingVoucherExpiryAnalyticsExports
                    ? "Refreshing..."
                    : "Refresh Scheduled Voucher Expiry Analytics Exports"}
                </button>
              </div>
              {voucherExpiryAnalyticsExportRuntime ? (
                <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                  <div>
                    <span className="font-medium text-slate-900">Runtime:</span>{" "}
                    {voucherExpiryAnalyticsExportRuntime.status} /{" "}
                    {voucherExpiryAnalyticsExportRuntime.message}
                  </div>
                  <div className="mt-1">
                    Format{" "}
                    {String(
                      voucherExpiryAnalyticsExportRuntime.details?.format ||
                        "json",
                    )}
                    , every{" "}
                    {String(
                      voucherExpiryAnalyticsExportRuntime.details
                        ?.interval_minutes || 0,
                    )}{" "}
                    minutes, retain{" "}
                    {String(
                    ...
                    , directory{" "}
                    {String(
                      voucherExpiryAnalyticsExportRuntime.details?.directory ||
                        "unset",
                    )}
                    .
                  </div>
                  ...
                  {voucherExpiryAnalyticsExportRuntime.details
                    ...
                    <div className="mt-1">
                      ...
                      {voucherExpiryAnalyticsExportRuntime.details?.next_due_at
                        ? `, next due ${String(voucherExpiryAnalyticsExportRuntime.details.next_due_at)}`
                        : ""}
                      .
                    </div>
                  ) : null}
                ...
                <div className="mt-3 rounded-md border border-dashed border-gray-300 px-3 py-4 text-xs text-gray-500">
                  No scheduled voucher expiry analytics export runtime has been
                  recorded yet.
                </div>
              )}
              {voucherExpiryAnalyticsExportArtifacts.length === 0 ? (
                <div className="mt-3 text-xs text-gray-500">
                  No scheduled voucher expiry analytics export artifacts are
                  present yet.
                </div>
              ) : (
                <div className="mt-3 overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 text-xs">
                    ...
                    <tbody className="divide-y divide-gray-100 bg-white">
                      {voucherExpiryAnalyticsExportArtifacts.map((artifact) => (
                        <tr key={artifact.name}>
                          <td className="px-3 py-2 text-gray-600">
                            {artifact.created_at}
                          </td>
                          ...
                          <td className="px-3 py-2">
                            <button
                              onClick={() =>
                                void downloadScheduledVoucherExpiryAnalyticsExport(
                                  artifact,
                                )
                              }
                              disabled={busyAction !== ""}
            ...
            <div className="rounded-md border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <button
                  onClick={() => void loadGuestSponsorAnalyticsExports(true)}
                  disabled={
                    loadingGuestSponsorAnalyticsExports || busyAction !== ""
                  }
                  className="rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50"
                >
                  {loadingGuestSponsorAnalyticsExports
                    ? "Refreshing..."
                    : "Refresh Scheduled Guest Sponsor Analytics Exports"}
                </button>
              </div>
              {guestSponsorAnalyticsExportRuntime ? (
                <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                  <div>
                    <span className="font-medium text-slate-900">Runtime:</span>{" "}
                    {guestSponsorAnalyticsExportRuntime.status} /{" "}
                    {guestSponsorAnalyticsExportRuntime.message}
                  </div>
                  <div className="mt-1">
                    Format{" "}
                    {String(
                      guestSponsorAnalyticsExportRuntime.details?.format ||
                        "json",
                    )}
                    , every{" "}
                    {String(
                    ...
                    , directory{" "}
                    {String(
                      guestSponsorAnalyticsExportRuntime.details?.directory ||
                        "unset",
                    )}
                    .
                  </div>
                  <div className="mt-1">
                    ...
                    buckets, limit{" "}
                    {String(
                      guestSponsorAnalyticsExportRuntime.details?.limit || 5000,
                    )}
                    .
                  </div>
                  {guestSponsorAnalyticsExportRuntime.details
                    ...
                    <div className="mt-1">
                      ...
                      {guestSponsorAnalyticsExportRuntime.details?.next_due_at
                        ? `, next due ${String(guestSponsorAnalyticsExportRuntime.details.next_due_at)}`
                        : ""}
                      .
                    </div>
                  ) : null}
                ...
                <div className="mt-3 rounded-md border border-dashed border-gray-300 px-3 py-4 text-xs text-gray-500">
                  No scheduled guest sponsor analytics export runtime has been
                  recorded yet.
                </div>
              )}
              {guestSponsorAnalyticsExportArtifacts.length === 0 ? (
                <div className="mt-3 text-xs text-gray-500">
                  No scheduled guest sponsor analytics export artifacts are
                  present yet.
                </div>
              ) : (
                <div className="mt-3 overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 text-xs">
                    ...
                    <tbody className="divide-y divide-gray-100 bg-white">
                      {guestSponsorAnalyticsExportArtifacts.map((artifact) => (
                        <tr key={artifact.name}>
                          <td className="px-3 py-2 text-gray-600">
                            {artifact.created_at}
                          </td>
